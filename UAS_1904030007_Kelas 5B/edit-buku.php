...
<?php
  ...
  $id = $_GET['id'];
  $edit = query("SELECT * FROM Table_Buku WHERE id=$id");
  //var_dump($edit);
            ...
            <div id="layoutSidenav_content">
                <main>
                    ...
                    <div class="col-md-10 p-5 pt-5">
                     <!-- konten -->
                    <h3><i class="fas fa-table"></i> Edit Data Buku </h3><hr>
                        <form method="POST" action="">
                            <input type="hidden" class="form-control" value="<?= $edit['id']?>" name="id">
                            <div class="mb-3">
                                <label for="No_Buku" class="form-label">No Buku :</label>
                                <input type="text" class="form-control" id="No_Buku" value="<?= $edit['No_Buku']?>" placeholder="No Buku" name="No_Buku" required autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <label for="Judul" class="form-label">Judul :</label>
                                <input type="text" class="form-control" id="Judul" value="<?= $edit['Judul']?>" placeholder="Judul" name="Judul" required autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <label for="ISBN" class="form-label">ISBN :</label>
                                <input type="text" class="form-control" id="ISBN" value="<?= $edit['ISBN']?>" placeholder="ISBN" name="ISBN" required autocomplete="off">
                            </div>
                            <div class="mb-3">
